...
- A PostgreSQL database (any of the following work):
  - Render free tier
  - Supabase
...
Open the application at [http://localhost:8501](http://localhost:8501)
...
- Final JSON payload (same shape as `/chat`)
...
FastAPI uses Uvicorn, which automatically selects a compatible event loop. If running async scripts directly on Windows, set the event loop policy before making async calls:
...
before starting the backend — otherwise the vectorstore MCP server will have no documents to search.
...
This application is intended for general educational and informational purposes only. It is not a substitute for professional medical advice, diagnosis, or treatment. Always consult a qualified healthcare provider for medical questions or concerns.
